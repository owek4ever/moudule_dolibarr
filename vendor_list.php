...
if ($action === 'confirm_delete' && $confirm === 'yes' && $socid_to_delete > 0) {
    if (empty($user->rights->societe->supprimer)) {
        ...
        if ($soc->fetch($socid_to_delete) > 0) {
            ...
            // 1) Delete every customer invoice attached to this thirdparty (in case it's also a client).
...
// Pull vendors from Dolibarr third-parties module (fournisseur = supplier flag).
// We map societe columns to the same aliases used throughout this page so that
// the rest of the code (display, sorting, filtering) keeps working unchanged.
//   t.ref      → soc.code_fournisseur  (supplier reference in third parties)
//   t.name     → soc.nom
//   t.phone    → soc.phone
//   t.email    → soc.email
//   t.type     → '' (third-parties have no flotte type; shown as '—')
//   t.website  → soc.url
//   t.note     → soc.note_public
//   t.address1 → soc.address
//   t.address2 → ''
//   t.city     → soc.town
//   t.state    → soc.zip
//   t.picture  → soc.logo
//   t.rowid    → soc.rowid  (used for card links → redirected to societe/card.php)

$sql  = "SELECT soc.rowid, ";
$sql .= " COALESCE(NULLIF(soc.code_fournisseur,''), CONCAT('TP-', soc.rowid)) AS ref,";
$sql .= " soc.nom AS name,";
$sql .= " soc.phone AS phone,";
$sql .= " soc.email AS email,";
$sql .= " '' AS type,";
$sql .= " soc.url AS website,";
$sql .= " soc.note_public AS note,";
$sql .= " soc.address AS address1,";
$sql .= " '' AS address2,";
$sql .= " soc.town AS city,";
$sql .= " soc.zip AS state,";
$sql .= " soc.logo AS picture";
$sql .= " FROM ".MAIN_DB_PREFIX."societe AS soc";
$sql .= " WHERE soc.fournisseur = 1";
$sql .= " AND soc.entity IN (".getEntity('societe').")";
$sql .= " AND soc.status = 1"; // active third-parties only

if ($search_ref) {
    $sql .= " AND soc.code_fournisseur LIKE '%".$db->escape($search_ref)."%'";
}
if ($search_name) {
    $sql .= " AND soc.nom LIKE '%".$db->escape($search_name)."%'";
}
if ($search_phone) {
    $sql .= " AND soc.phone LIKE '%".$db->escape($search_phone)."%'";
}
if ($search_email) {
    $sql .= " AND soc.email LIKE '%".$db->escape($search_email)."%'";
}
// $search_type has no equivalent in societe; skip to avoid empty results
if ($search_city) {
    $sql .= " AND soc.town LIKE '%".$db->escape($search_city)."%'";
}
if ($search_state) {
    $sql .= " AND soc.zip LIKE '%".$db->escape($search_state)."%'";
}

// Map flotte sort fields → societe columns
$sortfield_map = array(
    't.ref'     => 'soc.code_fournisseur',
    't.name'    => 'soc.nom',
    't.phone'   => 'soc.phone',
    't.email'   => 'soc.email',
    't.type'    => 'soc.nom',
...
$real_sortfield = isset($sortfield_map[$sortfield]) ? $sortfield_map[$sortfield] : 'soc.nom';
$sql .= $db->order($real_sortfield, $sortorder);

// Count total nb of records
$sqlcount = preg_replace('/^SELECT .+? FROM /s', 'SELECT COUNT(*) as nb FROM ', $sql);
// Strip the ORDER BY for the count query
...
if ($action === 'delete_confirm' && $socid_to_delete > 0) {
    ...
    $confirm_msg = $langs->trans("ConfirmDeleteVendor", $soc_tmp->name);
    $warn_parts = array();
...
<div class="vl-header">
    ...
    <div class="vl-header-actions">
        ...
        <a class="vl-btn vl-btn-primary" href="<?php echo dol_buildpath('/societe/card.php', 1); ?>?action=create&type=f">
...
<div class="vl-table-card">
    ...
    <table class="vl-table">
        ...
        <?php if (!empty($rows)) {
            foreach ($rows as $obj) {
                // Link to the third-party card in the societe module
            ...
            <tr>
                ...
                <td data-label="<?php echo $langs->trans('Action'); ?>">
                    <div class="vl-actions">
                        ...
                        <?php if ($user->rights->flotte->write) { ?>
                        <a href="<?php echo dol_buildpath('/societe/card.php', 1); ?>?socid=<?php echo $obj->rowid; ?>&action=edit" class="vl-action-btn edit" title="<?php echo $langs->trans('Edit'); ?>"><i class="fa fa-pen"></i></a>
                        <?php } ?>
                        <?php if ($user->rights->societe->supprimer) { ?>
                        <a href="<?php echo $self.'?action=delete&socid='.$obj->rowid.(!empty($param) ? '&'.$param : ''); ?>" class="vl-action-btn del" title="<?php echo $langs->trans('Delete'); ?>"><i class="fa fa-trash"></i></a>
                        <?php } ?>
                    </div>
                </td>
            </tr>
