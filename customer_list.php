...
$sqlcount = str_replace('SELECT t.rowid, t.ref, t.firstname, t.lastname, t.phone, t.email, t.company_name, t.tax_no, t.payment_delay, t.gender', 'SELECT COUNT(*) as nb', $sql);
...
if ($user->rights->flotte->write) {
    $newCardButton = dolGetButtonTitle($langs->trans('NewCustomer'), '', 'fa fa-plus-circle', dol_buildpath('/flotte/customer_card.php', 1).'?action=create', '', $user->rights->flotte->read);
}

print load_fiche_titre($langs->trans("CustomersList"), $newCardButton, 'user');

// Actions bar
print '<div class="tabsAction">'."\n";
if ($user->rights->flotte->write) {
    print '<a class="butAction" href="'.dol_buildpath('/flotte/customer_card.php', 1).'?action=create">'.$langs->trans("NewCustomer").'</a>'."\n";
}
...
// Print barre liste
print_barre_liste($langs->trans("CustomersList"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'user', 0, $newCardButton, '', $limit, 0, 0, 1);
...
print '<td class="liste_titre"></td>'; // Tax No
print '<td class="liste_titre"></td>'; // Payment Delay
...
print_liste_field_titre("Gender", $_SERVER["PHP_SELF"], "t.gender", "", $param, '', $sortfield, $sortorder);
...
if ($resql && $num > 0) {
    ...
    while ($i < min($num, $limit)) {
        ...
        print img_object($langs->trans("ShowCustomer"), "user", 'class="pictofixedwidth"');
        ...
        // Payment Delay
        print '<td>'.($obj->payment_delay ? $obj->payment_delay . ' ' . $langs->trans('Days') : '').'</td>';
        ...
        if ($user->rights->flotte->read) {
            print '<a class="editfielda" href="'.dol_buildpath('/flotte/customer_card.php', 1).'?id='.$obj->rowid.'" title="'.$langs->trans("View").'">'.img_view($langs->trans("View")).'</a>';
        }
        if ($user->rights->flotte->write) {
        ...
        if ($user->rights->flotte->delete) {
            print '<a class="editfielda" href="'.dol_buildpath('/flotte/customer_card.php', 1).'?id='.$obj->rowid.'&action=delete&token='.newToken().'" title="'.$langs->trans("Delete").'">'.img_delete($langs->trans("Delete")).'</a>';
        }
        print '</td>';
